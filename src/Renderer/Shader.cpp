...
uint32_t Shader::CompileShaders(uint32_t type, const std::string& source) {
  ...
  if (result == GL_FALSE) {
    ...
    std::cout << "Failed to compille " << (type == GL_VERTEX_SHADER ? "vertex" : "fragment") << " shader!" << std::endl;
...
uint32_t Shader::CreateShaders(const std::string& vertexShader, const std::string& fragmentShader) {
  GLCall(uint32_t program = glCreateProgram());
  uint32_t vs = CompileShaders(GL_VERTEX_SHADER, vertexShader);
  uint32_t fs = CompileShaders(GL_FRAGMENT_SHADER, fragmentShader);
...
int Shader::GetUniformLocation(const std::string& name) {
  if (m_UniformLocationCahe.find(name) != m_UniformLocationCahe.end()) return m_UniformLocationCahe[name];
